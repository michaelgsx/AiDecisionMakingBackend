...
**App Service:** set `AZURE_OPENAI_ENDPOINT`, `AZURE_OPENAI_API_KEY` (e.g. Key Vault `azure-openai-api-key`). Example deployment names for resource **`ai-reg-embedding`**: `AZURE_OPENAI_EMBEDDING_DEPLOYMENT=text-embedding-ada-002` (1536-dim, matches `db/azure_search_index_risk_records.json`), `AZURE_OPENAI_CHAT_DEPLOYMENT=gpt-4o-mini`. Use `AZURE_OPENAI_SKIP_CHAT=true` to run assess without the chat label step.

**Cloud vs local:** `backend/.env` is only for running the API on your laptop (`./mvnw spring-boot:run`). **Production** reads **Application settings** on the Web App (Azure Portal → App Service → **Configuration**). This repo’s GitHub Action **only deploys the JAR**; it does **not** push API keys. Put `AZURE_OPENAI_API_KEY` and the rest there (or Key Vault references). Optionally add a workflow step that runs `az webapp config appsettings set` using **GitHub Actions secrets** if you want keys managed in GitHub instead of the portal—still never commit real keys to the repo.
